...
class Device extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('guest/device/list', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('layouts/header', $data);
            $this->load->view('guest/device/add', $data);
            $this->load->view('layouts/footer');
            return;
        }
    ...
    public function authQr($device)
    {
        ...
        if (count((array) $data_device)) {
            ...
            if(!$status_I->success ){
                ...
                $qr = json_decode($qr);
                $updateQr = $this->device_model->update(["qr_code" => @$qr->data->qr], $data_device->id);
                ...
                if($qr){
                    ...
                    if ($qr->success) {
                        // print_r($qr->data->qr);
                        // die;
                        $this->base64_to_jpeg($qr->data->qr,FCPATH."/qrcode/".$data_device->api_key.".png");
                ...
                // $data_device = $this->device_model->getbyId($device);
                // print_r($data_device->qr_code);
                // die;
               
                if($status_I->data->status === "authenticated"){
                     redirect('./guest_devices');
                }
                $this->base64_to_jpeg( $data_device->qr_code,FCPATH."/qrcode/".$data_device->api_key.".png");
        ...
        $this->load->view('layouts/header', $data);
        $this->load->view('guest/device/authqr', $data);
        $this->load->view('layouts/footer');
    }
