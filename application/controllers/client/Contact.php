...
class Contact extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if($id_group){
            ...
            $data['devices'] = $this->contact_model->getWhere($start,$limit,$id_group);
        ...
        $this->load->view('client/contact/list', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('layouts/header', $data);
            $this->load->view('client/contact/add', $data);
            $this->load->view('layouts/footer');
            return;
        }
    ...
    public function import()
    {
        ...
        if ($this->form_validation->run() == false) {  
            $this->load->view('layouts/header', $data);
            $this->load->view('client/contact/import', $data);
            $this->load->view('layouts/footer');
            return;
        }
        if (isset($_FILES["file"]["name"])) {
            ...
            foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				for($row=2; $row<=$highestRow; $row++)
				{
                    ...
                    if($name != "" && $number != ""){
                        $id_group = $this->input->post('id_group');
                        if($this->contact_model->add($name,$number,$id_group,$api_key )){
                            $cek = $this->whatsva->checkNumber_server($number);
                            if($cek){
                                $dcek = json_decode($cek);
                            if($dcek->success){
    ...
    {   
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('layouts/header', $data);
            $this->load->view('client/contact/edit', $data);
            $this->load->view('layouts/footer');
            return;
        }
